...
#################################################
## REPRESENTATION-SPACE TRUST REGION (RSTR)   ##
## IMPALA + ICNN CRITIC                       ##
#################################################
...
#SBATCH --output=%u.rstr_impala_icnn_no_mu.%j.out
#SBATCH --requeue
...
#SBATCH --job-name=rstr-impala-icnn-no-mu
...
# Run training
# Note: Currently uses TRPO with ICNN critic (enforces convexity)
# Full RSTR algorithm implementation can be added later
srun --gres=gpu:1 python -m src.main --config configs/exp5_rstr_impala_icnn_no_mu.json
