...
#################################################
## EXP8: WARMUP400 VALUE HEAD COMPARISON       ##
## Minigrid Unlock-v0                          ##
## quadratic_latent | quadratic_latent_mumin   ##
## squared_norm on encoder latent Z            ##
#################################################
...
#SBATCH --output=%u.exp8_warmup_valueheads_minigrid.%j.out
#SBATCH --requeue
...
#SBATCH --job-name=exp8-warmup-valueheads
...
srun --gres=gpu:1 python -m src.main --config configs/exp8_rstr_impala_vae_strict_warmup400_quadratic_latent_minigrid.json
srun --gres=gpu:1 python -m src.main --config configs/exp8_rstr_impala_vae_strict_warmup400_quadratic_latent_mumin_minigrid.json
srun --gres=gpu:1 python -m src.main --config configs/exp8_rstr_impala_vae_strict_warmup400_squared_norm_minigrid.json
