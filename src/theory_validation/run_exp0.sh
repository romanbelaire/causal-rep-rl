#!/bin/bash
# Theory validation v2 — Exp 0 VAE expert bootstrap (standalone SBATCH):
#   1. Train VAE experts with stochastic eval until the 95% expert checkpoint is saved.
...
TV2_SEEDS=(42 43 44)
GENERATED_CONFIG="${TV2_LOG}/generated_configs/expert_vae_minigrid_stochastic_3000.json"
...
base = Path("configs/theory_validation/expert_vae_minigrid.json")
out = Path("outputs/theory_validation_v2/generated_configs/expert_vae_minigrid_stochastic_3000.json")
...
echo "=== Exp 0: VAE expert training (GELU, stochastic eval, 3000 epoch cap) ==="
for S in "${TV2_SEEDS[@]}"; do
  srun --gres=gpu:1 python -m src.main --config "${GENERATED_CONFIG}" --seed "$S"
done

echo "=== Exp 0: build VAE Z_ref (weights_expert.pt, else weights_final.pt) ==="
for S in "${TV2_SEEDS[@]}"; do
  run_dir="${TV2_LOG}/expert_vae_minigrid_seed${S}"
  if [[ -f "${run_dir}/weights_expert.pt" ]]; then
    ...
    echo "WARNING [exp0]: seed ${S} — no weights_expert.pt; build/validate use weights_final.pt"
  else
  ...
  python -m src.theory_validation.validate_z_ref \
    ...
    --z-ref "$TV2_ZREF/vae_seed${S}.pt" \
    --output "$TV2_ZREF/exp0_validation_vae_seed${S}.json"
done
