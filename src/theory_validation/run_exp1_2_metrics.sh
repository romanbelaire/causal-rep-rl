#!/bin/bash
# Exp 1–2: latent agents (RSTR + VAE), 3 seeds
#SBATCH --nodes=1
...
SEEDS=(42 43 44)
for S in "${SEEDS[@]}"; do
  srun --gres=gpu:1 python -m src.main --config configs/theory_validation/rstr_lconv_on_minigrid.json --seed "$S"
  srun --gres=gpu:1 python -m src.main --config configs/theory_validation/vae_no_repr_loss_minigrid.json --seed "$S"
done
