#!/bin/bash

#SBATCH --nodes=1
#SBATCH --cpus-per-task=4
#SBATCH --gres=gpu:1
#SBATCH --mem=32gb
#SBATCH --time=2-00:00:00
#SBATCH --array=0-11
#SBATCH --constraint=l40|l40s|a100|a40
#SBATCH --mail-type=END
#SBATCH --output=%u.perf_shared_hps_dmcontrol_s_%a.%j.out
#SBATCH --requeue
#SBATCH --partition=researchshort
#SBATCH --account=pradeepresearch
#SBATCH --qos=research-1-qos
#SBATCH --mail-user=rbelaire.2021@phdcs.smu.edu.sg
#SBATCH --job-name=shared-hps-dmcontrol

# Matched 8M × 3-seed parity runs at CTRO Optuna trial-8 shared HPs.
# Array: 3 agents × 4 tasks (agent = ID // 4, task = ID % 4).
# EXP_NAME:
#   exp_shared_baseline | exp_shared_latent_nolink | exp_shared_ctro
# CTRO uses alpha/beta warmup 500 epochs (~1M steps at buffer 2048).

module purge
module load Python/3.10.16-GCCcore-13.3.0
module load CUDA/12.6.0 cuDNN/9.5.0.50-CUDA-12.6.0 OpenMPI/5.0.3-GCC-13.3.0

export NVCC_FLAGS="-allow-unsupported-compiler"

cd /common/home/users/r/rbelaire.2021/causal-rep || exit 1
source rl-venv/bin/activate

export CPLUS_INCLUDE_PATH=$CPLUS_INCLUDE_PATH:~/LLM/llm/include/python3.10:/opt/apps/software/Python/3.10.16-GCCcore-13.3.0/include/python3.10
export LIBRARY_PATH=$LIBRARY_PATH:~/LLM/llm/lib:/opt/apps/software/Python/3.10.16-GCCcore-13.3.0/lib
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/opt/apps/software/Python/3.10.16-GCCcore-13.3.0/lib

export PYTORCH_CUDA_ALLOC_CONF="expandable_segments:True,max_split_size_mb:32"
export MUJOCO_GL=egl

pip install -q 'dm_control==1.0.38' 'mujoco==3.6.0' shimmy

mkdir -p results/slurm

nvidia-smi

ALL_TASKS=(cheetah-run walker-walk hopper-hop cartpole-swingup)
AGENTS=(baseline latent_nolink ctro)

AGENT_IDX=$((SLURM_ARRAY_TASK_ID / 4))
TASK_IDX=$((SLURM_ARRAY_TASK_ID % 4))
AGENT="${AGENTS[$AGENT_IDX]}"
TASKS=("${ALL_TASKS[$TASK_IDX]}")
SEEDS=(42 43 44)
SUITE=dmcontrol_state
RESULTS_SUBDIR=dmcontrol_state

SHARED=(--learning-rate 1.0128e-4 --entropy-coef 0.0408 --num-epochs 20 --policy-hidden 256,256)

case "${AGENT}" in
  baseline)
    EXP_NAME=exp_shared_baseline
    EXTRA_ARGS=(--agent ppo "${SHARED[@]}")
    ;;
  latent_nolink)
    EXP_NAME=exp_shared_latent_nolink
    EXTRA_ARGS=(--agent ctro --alpha 0 --beta 0 "${SHARED[@]}")
    ;;
  ctro)
    EXP_NAME=exp_shared_ctro
    EXTRA_ARGS=(
      --agent ctro
      --alpha 0.00205
      --beta 0.532
      --alpha-warmup-epochs 500
      --beta-warmup-epochs 500
      "${SHARED[@]}"
    )
    ;;
  *)
    echo "Unknown AGENT=${AGENT}" >&2
    exit 1
    ;;
esac

ENVS_PER_PROC="${ENVS_PER_PROC:-1}"
THREADS_PER_PROC="${THREADS_PER_PROC:-1}"
MAX_PARALLEL="${MAX_PARALLEL:-3}"

echo "Shared HPs: AGENT=${AGENT} EXP_NAME=${EXP_NAME} TASK=${TASKS[0]} EXTRA_ARGS=${EXTRA_ARGS[*]}"

source src/experiments/jobs/_parallel_seeds.sh
run_training_pool
exit $?
