...
#SBATCH --output=%u.perf_stress_onz_dmcontrol_s_%a.%j.out
#SBATCH --requeue
#SBATCH --partition=researchshort
#SBATCH --account=pradeepresearch
#SBATCH --qos=research-1-qos
#SBATCH --mail-user=rbelaire.2021@phdcs.smu.edu.sg
#SBATCH --job-name=stress-onz-dmcontrol

# On-Z stress: hunt PPO-on-Z failure (μ_PL drop + return drop) vs CTRO hold.
# Agents: latent_nolink + CTRO only (array 2×4 tasks).
# Stress knobs on shared torso width: ent=0, lr=5e-4, epochs=40.
# CTRO keeps t8 alpha/beta (protection on); latent_nolink keeps 0/0.
# EXP_NAME: exp_stress_latent_nolink | exp_stress_ctro
...
STRESS=(--learning-rate 5e-4 --entropy-coef 0 --num-epochs 40 --policy-hidden 256,256)

case "${AGENT}" in
  latent_nolink)
    EXP_NAME=exp_stress_latent_nolink
    EXTRA_ARGS=(--agent ctro --alpha 0 --beta 0 "${STRESS[@]}")
    ;;
  ctro)
    EXP_NAME=exp_stress_ctro
    EXTRA_ARGS=(--agent ctro --alpha 0.00205 --beta 0.532 "${STRESS[@]}")
    ;;
  *)
...
echo "Stress on-Z: AGENT=${AGENT} EXP_NAME=${EXP_NAME} TASK=${TASKS[0]} EXTRA_ARGS=${EXTRA_ARGS[*]}"
