...
#SBATCH --job-name=ctro-perf-procgen

# Procgen easy performance eval (8 games, full + test distributions).
#
...
#   EXP_NAME=exp_full SEEDS="42 43 44" sbatch perf_eval_procgen_s.sh
...
if [ ! -f "${PROBE_CKPT}" ]; then
  ...
  echo "Run training first: sbatch src/experiments/jobs/perf_train_procgen_s.sh"
  exit 1
fi
