...
#SBATCH --output=%u.perf_eval_procgen_latent_nolink_s_.%j.out
#SBATCH --requeue
#SBATCH --partition=researchshort
#SBATCH --account=pradeepresearch
#SBATCH --qos=research-1-qos
#SBATCH --mail-user=rbelaire.2021@phdcs.smu.edu.sg
#SBATCH --job-name=nolink-perf-procgen

# Procgen easy latent-no-link performance eval (8 games, full + test distributions).
# Checkpoints from CTRO CNN-VAE stack trained with alpha=0, beta=0.
#
...
#   EXP_NAME=exp_latent_nolink SEEDS="42 43 44" sbatch perf_eval_procgen_latent_nolink_s.sh
...
if [ ! -f "${PROBE_CKPT}" ]; then
  ...
  echo "Run training first: sbatch src/experiments/jobs/perf_train_procgen_latent_nolink_s.sh"
  exit 1
fi
