...
## 2026-09-10 — Procgen frozen predictiveness re-rollout — job 45726633 — done

CPU RM-shared job 45726633 finished in 00:07:02 (COMPLETED 0:0) with all 90 Procgen easy cells re-rolled. Seven cells had median pairwise latent distance \(s_Z=0\) and were recorded as fully aliased. Output: `results/aliasing_predictiveness/procgen_easy/{cells.csv,summary.json}`.

The gate is whether local aliasing predicts held-out test return (`eval_test_return_mean`, Procgen `num_levels=0`) more strongly than participation ratio (PR). Averaging Spearman coefficients computed inside each game (so coinrun’s return scale cannot dominate starpilot): logged PR +0.30, same-batch re-rolled PR +0.46, fifth-percentile pairwise distance +0.45. Hard aliasing rate is −0.24 (right sign: more aliased pairs, lower return) and does not beat either PR. The \(\alpha\)-free soft score is −0.11; geometry–value Spearman is +0.21; probe normalized mean squared error is +0.12, which is the wrong sign for “worse fit, worse return.” On dense games the aliasing-rate coefficient is −0.45 versus logged PR +0.48 and re-rolled PR +0.74; on sparse games every predictor is near zero. Fifth-percentile distance matches same-batch PR and is stronger than logged PR, but that is a collapse/spread statistic, not return-conditioned aliasing. Contribution 1 as locked does not hold. Do not submit the 30-cell GPU hinge screen.

## 2026-09-10 — frozen predictiveness re-rollout — job 45716019 — done

CPU RM-shared job 45716019 finished in 00:04:36 (COMPLETED). It did not complete the Procgen re-rollout that gates contribution 1. The first Python command crashed at Procgen cell 32/90 (`exp_ctro_cnn` seed 43 leaper) because the median pairwise latent distance \(s_Z\) was 0, which `aliasing_diagnostics` treated as an error. The job script had no `set -e`, so DeepMind Control Suite (DMC) state re-rollouts and the Phoenix leading-indicator script still ran. Root `results/aliasing_predictiveness/{cells.csv,summary.json}` are still the earlier CSV-only join (90 Procgen cells, no `reroll_*` columns).

DMC state finished 60 cells (five loadable arms × four tasks × three seeds). Pooled Spearman of re-rolled functional value-probe mean squared error (MSE) with `eval_test_return_mean` is +0.55, versus +0.07 for participation ratio (PR). That probe coefficient is the wrong sign for “aliasing predicts failure,” and it is a scale confound: hopper-hop cells with near-zero return also have near-zero GAE targets, so raw probe MSE is tiny even when the encoder is uninformative. Ranking within task first, then correlating, gives probe MSE +0.03, aliasing rate −0.20, and PR −0.20. Mean of the four within-task Spearmans is the same. Cartpole-swingup is the only task where local aliasing looks like the hypothesis (aliasing rate −0.90 vs PR −0.58). Cheetah, hopper, and walker do not. DMC `eval_test` is not Procgen held-out levels, so this suite cannot pass the paper gate even if the signs had lined up.

Phoenix Stage A ep16 numbers are unchanged: contemporaneous Spearman of on-policy cumulant normalized MSE with eval return about −0.23; lag-10 near 0. Contribution 1 is not in hand. Do not submit the 30-cell GPU hinge screen. Procgen still needs a CPU re-rollout after recording \(s_Z=0\) as total aliasing rather than aborting the matrix.

## 2026-09-10 — anti-aliasing hinge + predictiveness gate — no GPU job — pending

Contribution 1 is predictiveness, not sensitivity. Job 43539537 showed fifth-percentile pairwise distance moving while participation ratio did not (p = 0.835); the arms did not differ reliably in return, so that run does not show that local geometry predicts failure. Failure is locked as held-out test return (`eval_test_return_mean`, Procgen `num_levels=0`). MiniGrid `plots/ctro/ablation_table.txt` reports SEM across three seeds of final eval return; the `{0,0,3m}` pattern makes mean = SEM in three of four cells, pairwise |t| < 1, and FULL was selected by peeking at those same evals. Do not quote MICO_ONLY < BASELINE < PL_ONLY < FULL. Ocean still has 96 Procgen finals and matched DMC finals; there are no intermediate weights, so aliasing at time t other than T requires a re-rollout from `weights_final.pt`. Code for the median-normalized GAE hinge, A0/A1/A2 screen jobs, and the frozen re-analysis is in this tree. Do not submit the 30-cell GPU screen until the re-analysis shows local geometry predicting test return better than participation ratio. Job 45716019 did not produce that result: Procgen re-rollout crashed, and the finished DMC-state cells do not show probe MSE beating PR after removing task-scale confounding. Job 45726633 completed the Procgen re-rollout: return-conditioned aliasing (hard rate, soft score, value Spearman, probe NMSE) does not beat PR; fifth-percentile pairwise distance matches same-batch PR but is not aliasing. Reward density (dense: starpilot, fruitbot, caveflyer, chaser; sparse: coinrun, maze, miner, leaper) is the pre-registered interaction on that predictor, not a fourth finding.
