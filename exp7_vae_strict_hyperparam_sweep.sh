...
#################################################
## VAE STRICT (repr loss) — HYPERPARAM SWEEP   ##
...
#SBATCH --output=%u.vae_strict_sweep.%j.out
#SBATCH --requeue
...
#SBATCH --job-name=vae-strict-sweep
...
srun --gres=gpu:1 python -m src.main --config configs/exp4_rstr_impala_vae_strict_soft.json
srun --gres=gpu:1 python -m src.main --config configs/exp4_rstr_impala_vae_strict_warmup400.json
srun --gres=gpu:1 python -m src.main --config configs/exp4_rstr_impala_vae_strict_reprmatch.json
